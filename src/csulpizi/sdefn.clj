(ns csulpizi.sdefn
  ...
  (:require [csulpizi.sdefn.arguments :refer [args-sym sdefn-args->arg-map]]
            [csulpizi.sdefn.utils :refer [defmacro*]]
            [csulpizi.sdefn.validation :refer [validate-params]]))

(defmacro sdefn*
  "Creates a macro that checks whether or not  expand to (apply f args) if the requi"
  [f# {:keys [name# doc-string# attribute-map# defn-params# required-keys#]}]
  (list `defmacro* name# doc-string# attribute-map# defn-params#
        (list `validate-params required-keys# args-sym)
